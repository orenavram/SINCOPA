...
LocusSequencesSimulator::LocusSequencesSimulator(LocusTreeStructure & locus_tree_structure, size_t locus_len) : _locus_tree_structure(locus_tree_structure), _locus_len(locus_len){
}

void LocusSequencesSimulator::simulate(const double p_a, const double p_c, const double p_g, const double p_transition){
	...
	if(PRINT_MODE){cout << "with sequence: " << endl << _sequences[lca_allele] << endl;}
	...
	while(queue.size() > 0){
		...
		for(map<int, int>::iterator iterator = _locus_tree_structure._allelic_tree.begin(); iterator != _locus_tree_structure._allelic_tree.end(); iterator++) {
			...
			if(ancestor == lca_allele){
				if(PRINT_MODE){cout << "Handling branch from " << ancestor << " to " << descendant << " with " << _locus_tree_structure._num_of_mutations_tree[descendant] << " mutations." <<endl;}
				...
				if(PRINT_MODE){cout << descendant << " was mutated from this: " << _sequences[descendant];}

				mutate_sequence(descendant, p_transition);
				
				if(PRINT_MODE){cout << " to:" << _sequences[descendant] << endl;}
				...
				if(PRINT_MODE){cout << "Allele "<< descendant <<" was generated." <<endl;}
...
void LocusSequencesSimulator::mutate_sequence(const size_t allele, const double p_transition){
...
void LocusSequencesSimulator::mutate_nucleotide(const size_t random_nucleotide_index, const int allele, const double p_transition){
...
string LocusSequencesSimulator::draw_root_sequence(const double p_a, const double p_c, const double p_g) const {
	double random_event;
...
char LocusSequencesSimulator::apply_transition(const char nuc) const{
	switch(nuc){
		case 'a': 
			return 'g';
		case 'g': 
...
char LocusSequencesSimulator::apply_transversion(const char nuc) const{
...
void LocusSequencesSimulator::check_data_validity3(void){
	for(map<size_t, string>::iterator iterator = _sequences.begin(); iterator != _sequences.end(); iterator++) {
		if(iterator->second.size() != _locus_len){
			cout << endl << "error!!!" << endl;
			cout << "iterator->second is " << iterator->second << endl;
