...
LocusTreeStructure::LocusTreeStructure(const vector< vector<int> > & mutations_history, vector<int> current_alleles, size_t locus_index, size_t total_num_of_bacteria, size_t pop_size){
	
	//current_alleles will maintain the alleles state at every generation when going back in time
	...
	while(there_is_more_than_one_allele(last_coalecsent)){
		...
		if(DEBUG_MODE){cout << "last_coalecsent.size() = " << last_coalecsent.size() << endl;}
		...
		parent_allele = mutations_history[locus_index][max_allele];
...
int LocusTreeStructure::get_oldest_allele(void){
